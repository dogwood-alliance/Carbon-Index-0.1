...
`r params$stateText ` has `r stSumFt` forest types across the state, with `r stMaxAc_FT` acres being of the `r stMaxAc_Name`. There are only `r stMinAc_FT` acres of the `r stMinAc_Name`. The `r stMaxFCI_Name` has the highest FCI at `r stMaxFCI_FT` while the `r stMinFCI_Name` has the lowest FCI at `r stMinFCI_FT`.  Overall, the total FCI in `r params$stateText ` is `r stTotalFCI `.
