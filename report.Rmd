...
# Forest Types  
```{r echo= FALSE}
#stSumFt<- n_distinct(data$Forest.type.group)
```

`r params$stateText ` has StSumFt forest types across the state, with <<StMaxFT_AcPerc>>% of acres being <<StMaxAcFT_Name>>. There are only <<StMinAcFT_Ac>> acres of <<StMinAcFT_Name>>. The <<STMaxFT_Name>> has the highest FCI at <<StMaxFT_FCI>> while the <<StMinFT_Name>> has the lowest FCI at <<StMinFT_FCI>>.  Overall, the total FCI in `r params$stateText ` is <<StTotalFCI>>.
